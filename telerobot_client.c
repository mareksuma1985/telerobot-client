...
/* tworzy ikonki */
GtkWidget *quit_icon, *fullscreen_enable_icon, *fullscreen_disable_icon, *video_icon, *joystick_icon;
/* tworzy przyciski */
GtkWidget *quit_button, *fullscreen_enable_button, *fullscreen_disable_button;
/* tworzy checkboxy */
GtkWidget *check_button_video, *check_button_joystick, *check_button_serwokontroler, *check_button_regulator;
...
if (bytes_sent < 0)
{printf("Error sending packet: %s\n", strerror(errno));}
 printf("%2hd△ numer osi:%2hd\twychylenie:%6hd\n", bytes_sent, (packet_stick_UDP[0]-128)/16, (packet_stick_UDP[0]%16)*64 + packet_stick_UDP[1]-128);
}
...
/* wysyła na gniazdo sock, pakiet packet_dwustan_UDP, o długosci strlen(packet_dwustan_UDP)... */
...
void wyslij_dwustan(unsigned char number, short value)
{
	...
	{
	case 0: printf("przycisk %d, stan %d, tryb: 2\n", number, value);
	break;  
	case 1: wyslij_dwustan_UDP(number, value); /* wysyła protokołem UDP */
	break;  
	case 2: printf("przycisk %d, stan %d, tryb: 2\n", number, value); /* nie użyta */
	break;
	case 3: printf("przycisk %d, stan %d, tryb: 3\n", number, value); /* nie użyta */
	break; 
	}
}

/* przełącza stan checkboxa */
void toggle_serwokontroler_button_callback (GtkWidget *check_button_serwokontroler, gpointer data)
{
    if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (check_button_serwokontroler))) 
    {
    wyslij_dwustan (14,1);
    /* wysyła sygnał załączenia serwokontrolera */
	push_item(statusbar, GINT_TO_POINTER (context_id), "serwokontroler włączony");
    ...
    push_item(statusbar, GINT_TO_POINTER (context_id), "serwokontroler wyłączony");
	/* wyświetla wiadomość na pasku stanu */	
    }
}

/* przełącza stan checkboxa */
void toggle_regulator_button_callback (GtkWidget *check_button_regulator, gpointer data)
{
    if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (check_button_regulator))) 
			...
			push_item(statusbar, GINT_TO_POINTER (context_id), "regulator wyłączony");
			}
}
...
void toggle_joystick_button_callback (GtkWidget *check_button_joystick, gpointer data)
{
    if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (check_button_joystick))) 
    {wykryj_joystick();}
    else
    {zamknij_joystick();}
}
...
/* jeśli wychylenie joysticka w osi Y jest inne niż 0 to poniższe instrukcje są powtarzane regularnie, zależnie od wybranego trybu wysyłania */
	switch (tryb_wysylania)
	{ 
	case 0: printf("oś %d, wychylenie %d, tryb: 2\n", 1, stick_y);
	break;  
	case 1: wyslij_stick_UDP(1, stick_y);
	break;  
	case 2: printf("oś %d, wychylenie %d, tryb: 2\n", 1, stick_y);
	break;
	case 3: printf("oś %d, wychylenie %d, tryb: 3\n", 1, stick_y);
	break;
	}
}
...
void wyslij_stick(unsigned int number, short value)
{
	...
	case 0: printf("oś %d, wychylenie %d, tryb: 2\n", number, value);
	break;  
	case 1: wyslij_stick_UDP(number, value);
	break;  
	case 2: printf("oś %d, wychylenie %d, tryb: 2\n", number, value);
	break;
	case 3: printf("oś %d, wychylenie %d, tryb: 3\n", number, value);
...
void klawisz(GtkWidget *widget,  GdkEventKey *event, int stan)
{ /* parametr stan jest przekazywany do funkcji i przyjmuje wartości 1 lub 0 */ 
...
switch (jse.number)	{
			case 0: /* wyslij_dwustan(0,1); */
				break;
			case 1: wyslij_dwustan(1,1);
				break;
			case 2: /* wyslij_dwustan(2,1); */
				break;
			case 3: wyslij_dwustan(3,1);
				break;
			case 4: wyslij_dwustan(4,1);
			play_rumble_effect(RUMBLE_DAMPING_EFFECT);
				break;
			case 5: wyslij_dwustan(5,1);
			play_rumble_effect(RUMBLE_SPRING__EFFECT);
				break;
			case 6: wyslij_dwustan(6,1);
			play_rumble_effect(RUMBLE_SINE_VIBE_EFFECT);
				break;
			case 7: wyslij_dwustan(7,1);
			play_rumble_effect(RUMBLE_CONSTANT_FORCE_EFFECT);
				break;
			case 8: wyslij_dwustan(8,1);
			stop_all_rumble_effects();
				break;
			case 9: wyslij_dwustan(9,1);
				break;
			case 10: wyslij_dwustan(10,1); /* wciśnięcie lewego joysticka */
				break;
			case 11: wyslij_dwustan(11,1); /* wciśnięcie prawego joysticka */
				break;
			default: printf("przycisk %u nacisniety\n", jse.number);
				break;
			}
...
switch (jse.number) 	{ 
			case 0: /* wyslij_dwustan(0,0); */
				break;
			case 1: wyslij_dwustan(1,0);
				break;
			case 2: /* wyslij_dwustan(2,0); */
				break;
			case 3: wyslij_dwustan(3,0);
				break;
			case 4: wyslij_dwustan(4,0);
			stop_all_rumble_effects();
				break;
			case 5: wyslij_dwustan(5,0);
			stop_all_rumble_effects();
				break;
			case 6: wyslij_dwustan(6,0);
			stop_all_rumble_effects();
				break;
			case 7: wyslij_dwustan(7,0);
			stop_all_rumble_effects();
				break;
			case 8: wyslij_dwustan(8,0);
				break;
			case 9: wyslij_dwustan(9,0);
				break;
			case 10: wyslij_dwustan(10,0); /* zwolnienie lewego joysticka */
				break;
			case 11: wyslij_dwustan(11,0); /* zwolnienie prawego joysticka */
				break;
			default: printf("przycisk %u zwolniony\n", jse.number);
				break;
			}
...
int wykryj_joystick()
{
	...
	printf("Nie wykryto joysticka %s\n", joystick_descriptor);
	  ...
	  g_signal_connect(check_button_joystick, "clicked", G_CALLBACK(toggle_joystick_button_callback), NULL );
	  /* FIXME */
	  }
	}
	else
...
drawing_area_frame= gtk_aspect_frame_new(NULL, 0.5, 0.0, 1.33333, TRUE /* ignore child's aspect */);
...
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_9, 2, 3, 0, 1 );
GtkWidget *button_num_arrow_8;
button_num_arrow_8 = gtk_button_new_with_label("▲▲");  
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_8, 1, 2, 0, 1 );
GtkWidget *button_num_arrow_7;
button_num_arrow_7 = gtk_button_new_with_label("▢▲");  
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_7, 0, 1, 0, 1 );
GtkWidget *button_num_arrow_4;
button_num_arrow_4 = gtk_button_new_with_label("▼▲");  
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_4, 0, 1, 1, 2 );
GtkWidget *button_num_arrow_5;
button_num_arrow_5 = gtk_button_new_with_label("▢▢");  
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_5, 1, 2, 1, 2 );
GtkWidget *button_num_arrow_6;
button_num_arrow_6 = gtk_button_new_with_label("▲▼");  
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_6, 2, 3, 1, 2 );
GtkWidget *button_num_arrow_3;
button_num_arrow_3 = gtk_button_new_with_label("▢▼");  
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_3, 2, 3, 2, 3 );
GtkWidget *button_num_arrow_2;
button_num_arrow_2 = gtk_button_new_with_label("▼▼");  
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_2, 1, 2, 2, 3 );
GtkWidget *button_num_arrow_1;
button_num_arrow_1 = gtk_button_new_with_label("▼▢");  
gtk_table_attach_defaults(GTK_TABLE(left_table), button_num_arrow_1, 0, 1, 2, 3 );
...
gtk_table_attach_defaults(GTK_TABLE(main_table), tank_controlls, 0,1,0,1 );
...
g_signal_connect(button_num_arrow_9, "pressed", G_CALLBACK(num_arrow_9), NULL);
g_signal_connect(button_num_arrow_9, "released", G_CALLBACK(no_num_arrow), NULL);

g_signal_connect(button_num_arrow_8, "pressed", G_CALLBACK(num_arrow_8), NULL);
g_signal_connect(button_num_arrow_8, "released", G_CALLBACK(no_num_arrow_y), NULL);

g_signal_connect(button_num_arrow_7, "pressed", G_CALLBACK(num_arrow_7), NULL);
g_signal_connect(button_num_arrow_7, "released", G_CALLBACK(no_num_arrow), NULL);
            
g_signal_connect(button_num_arrow_6, "pressed", G_CALLBACK(num_arrow_6), NULL);
g_signal_connect(button_num_arrow_6, "released", G_CALLBACK(no_num_arrow_x), NULL);

g_signal_connect(button_num_arrow_5, "pressed", G_CALLBACK(no_num_arrow), NULL); 
g_signal_connect(button_num_arrow_5, "released", G_CALLBACK(no_num_arrow), NULL); 
/* g_signal_connect(button_num_arrow_5, "clicked", G_CALLBACK(no_num_arrow), NULL);  */

g_signal_connect(button_num_arrow_4, "pressed", G_CALLBACK(num_arrow_4), NULL);
g_signal_connect(button_num_arrow_4, "released", G_CALLBACK(no_num_arrow_x), NULL);

g_signal_connect(button_num_arrow_3, "pressed", G_CALLBACK(num_arrow_3), NULL);
g_signal_connect(button_num_arrow_3, "released", G_CALLBACK(no_num_arrow), NULL);

g_signal_connect(button_num_arrow_2, "pressed", G_CALLBACK(num_arrow_2), NULL);
g_signal_connect(button_num_arrow_2, "released", G_CALLBACK(no_num_arrow_y), NULL);

g_signal_connect(button_num_arrow_1, "pressed", G_CALLBACK(num_arrow_1), NULL);
g_signal_connect(button_num_arrow_1, "released", G_CALLBACK(no_num_arrow), NULL);

gtk_signal_connect(GTK_OBJECT(main_window),"key_press_event", G_CALLBACK(klawisz), 1);
gtk_signal_connect(GTK_OBJECT(main_window),"key_release_event", G_CALLBACK(klawisz), 0);
...
/* gtk_button_set_focus_on_click(quit_button, 0); */
	...
	g_signal_connect(fullscreen_enable_button, "clicked", G_CALLBACK(switch_Fullscreen), NULL );
	gtk_table_attach_defaults(GTK_TABLE(right_table), fullscreen_enable_button, 2, 3, 1, 2 );
	...
	g_signal_connect(fullscreen_disable_button, "clicked", G_CALLBACK(switch_Fullscreen), NULL );
	gtk_table_attach_defaults(GTK_TABLE(right_table), fullscreen_disable_button, 2, 3, 1, 2 );
...
g_signal_connect(check_button_video, "clicked", G_CALLBACK(toggle_video), NULL);
gtk_table_attach_defaults(GTK_TABLE(right_table), video_icon, 1, 2, 2, 3 );
gtk_table_attach_defaults(GTK_TABLE(right_table), check_button_video, 1, 2, 2, 3 );
...
gtk_table_attach_defaults(GTK_TABLE(right_table), joystick_icon, 0, 1, 2, 3 );
gtk_table_attach_defaults(GTK_TABLE(right_table), check_button_joystick, 0, 1, 2, 3 );
...
g_signal_connect(check_button_serwokontroler, "clicked", G_CALLBACK(toggle_serwokontroler_button_callback), NULL);
gtk_table_attach_defaults(GTK_TABLE(right_table), check_button_serwokontroler, 0, 1, 1, 2 );
check_button_regulator= gtk_check_button_new_with_label ("reg");
gtk_table_attach_defaults(GTK_TABLE(right_table), check_button_regulator, 1, 2, 1, 2 );
gtk_toggle_button_set_active( check_button_regulator, TRUE );
g_signal_connect(check_button_regulator, "clicked", G_CALLBACK(toggle_regulator_button_callback), NULL);
